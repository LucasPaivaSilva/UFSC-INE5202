...
clear
clc
%f(x) tabelada aproximado por 3 trechos de Splines Cúbicas (APROXIMA SOMENTE FUNÇÕES), conforme os pontos abaixo:
x=[	  0.1	     0.4	      0.7         1];
y=[	 -2.30259	 -0.916291	  -0.356675	  0];
n=length(x)-1;
...
np=3*n;    %4 sub-divisões para cada sub-intervalo entre x(i) e x(i+1) para plotagem
xpp=[];ypp=[];
for i=1:n
