...
%teremos 'n' splines cúbicas, uma para cada intervalo, formando um sistema tridiagonal de 'm-1' equacoes para S (derivadas de segunda ordem) 
for i=1:n	
	h(i)=x(i+1)-x(i);
end
...
%ultima eq. i=n
t(n)=h(n-1); r(n)=(2*h(n-1)+3*h(n)); d(n)=0;   b(n)=6*((y(n+1)-y(n))/h(n)-(y(n)-y(n-1))/h(n-1)); %para Sn
